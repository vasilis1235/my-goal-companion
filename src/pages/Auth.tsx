...
const Auth = () => {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (error) toast.error("Λάθος email ή κωδικός");
    else navigate("/");
  };
  ...
  const handleGoogle = async () => {
    ...
    if (result.error) {
      toast.error("Σφάλμα Google login");
      setLoading(false);
      return;
    }
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-background via-background to-accent">
      <Card className="w-full max-w-md shadow-2xl border-border">
        <CardHeader className="text-center">
          ...
          <CardDescription>Καλώς ήρθες στον AI σύμμαχό σου</CardDescription>
        </CardHeader>
        <CardContent>
          <Button
            type="button"
          ...
          <div className="relative mb-4">
            ...
            <div className="relative flex justify-center text-xs"><span className="bg-card px-2 text-muted-foreground">Ή</span></div>
          </div>
